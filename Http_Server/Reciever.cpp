...
void Reciever:: receiveRequest(char *request, int request_size, int client_socketfd) {
    printf("=============================================\n");
...
void Reciever::openFileWithPathAndSend(string file_path, int client) {
    ...
    if (file)//if i found the file i can send it back to browser
    {
        ...
        p->sendFile(file, client, statusLine);
    }
...
void Reciever::parse_data_from_file(char *buffer, int buffer_size, int &data_start_position, int &data_content_length) {
...
void Reciever::receiveGETRequest(int client, int request_size) {
    char* buffer = (char*) malloc(request_size);
    int val_read = read(client , buffer, request_size);
    string request(buffer);
    string file_path = p->parse_req(request, 2);
    openFileWithPathAndSend(file_path, client);
}

void Reciever::receivePOSTRequest(int client_socketfd) {
    ...
    string req(buffer);
    char *file_url = strdup(p->parse_req(req, 2).c_str());
    ...
    p->sendBufferToSocket(strdup(OkResponse.c_str()), OkResponse.size(),client_socketfd);
