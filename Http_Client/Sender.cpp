...
void Sender::sendRequest(Receiver::requestType type, char* hostname, char * port_number, char * file_url, int client_socketfd) {
    if(type == Receiver::GET){
...
void Sender::sendGETRequest(char* hostname, char * port_number, char * file_url, int client_socketfd) {
    ...
    puts(request_buffer);
    if(length > 0){
        int status = p->sendBufferToSocket(request_buffer, length, client_socketfd);
        if(status == 0){
...
void Sender::sendPOSTRequest(char* hostname, char * port_number, char * file_url, int client_socketfd) {
    ...
    memset(request_buffer, '\0', sizeof(request_buffer));
    // Send POST Header
    int post_header_length = sprintf(request_buffer, "POST %s HTTP/1.1\r\nHOST: %s\r\nContent-Length: %d\r\n\r\n",file_url, hostname,p->getFileLength(file));
    puts(request_buffer);
    p->sendBufferToSocket(request_buffer, post_header_length, client_socketfd);
...
void Sender::waitUnitOK(int client_socketfd, FILE* file){
    struct timeval tv;
    ...
    p->sendFile(file, client_socketfd, "");
}
